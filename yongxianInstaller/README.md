...
> 🚀 一键部署你的私人 AI 助手 YongXian（涌现），支持多平台多模型配置

<p align="center">
  <img src="photo/menu.png" alt="YongXian 配置中心" width="600">
</p>
...
### 🖥️ 桌面版 YongXian Manager（推荐）

如果你更喜欢图形界面，推荐使用 **YongXian Manager** 桌面应用：

<p align="center">
  <a href="https://github.com/insight68/yongxian">
    <img src="https://img.shields.io/badge/下载桌面版-YongXian%20Manager-blue?style=for-the-badge&logo=github" alt="Download">
  </a>
</p>
...
👉 **下载地址**: [github.com/insight68/yongxian](https://github.com/insight68/yongxian)
...
```bash
# 1. 克隆仓库
git clone https://github.com/insight68/yongxian.git
cd yongxian
...
# 或从 GitHub 下载运行
curl -fsSL https://raw.githubusercontent.com/insight68/yongxian/main/config-menu.sh | bash
```
...
- **Anthropic Claude** - claude-sonnet-4-5 / claude-opus-4-5 / claude-haiku-4-5 _(支持自定义 API 地址)_
- **OpenAI GPT** - gpt-4o / gpt-4o-mini / gpt-4-turbo _(支持自定义 API 地址，需支持 v1/responses)_
...
> 💡 **无需公网服务器**：YongXian 使用飞书的 WebSocket 长连接模式接收事件，无需配置 Webhook 地址。
...
7. **在配置菜单中配置飞书**：输入 App ID 和 App Secret，启动 YongXian 服务
8. 配置"事件订阅"（使用长连接）：
   - 进入：事件与回调 → 选择「**使用长连接接收事件**」
   - 添加事件：`im.message.receive_v1`（接收消息）
   - **无需填写 Webhook 地址**
   - ⚠️ **注意**：需要 YongXian 服务已启动，才能保存长连接设置
9. 添加机器人到群组：群设置 → 群机器人 → 添加机器人

### 配置 WhatsApp

> 💡 **无需 Business API**：YongXian 通过扫码登录你的 WhatsApp 账号，无需申请 Business API。
...
- **YongXian 配置**: `~/.openclaw/yongxian.json` - YongXian 内部配置（自动管理）
...
当使用自定义 API 地址时，安装脚本会自动在 `~/.openclaw/yongxian.json` 中配置自定义 Provider：
...
```
~/.openclaw/
├── openclaw.json        # YongXian 核心配置
├── env                  # 环境变量 (API Key 等)
├── backups/             # 配置备份
└── logs/                # 日志文件 (由 YongXian 管理)
```

## 🛡️ 安全建议

> ⚠️ **重要警告**：YongXian 需要完全的计算机权限，请务必注意安全！
...
- [YongXian 官网](https://clawd.bot)
- [官方文档](https://clawd.bot/docs)
- [🖥️ YongXian Manager 桌面版](https://github.com/insight68/yongxian) - 图形界面管理工具
- [安装工具仓库](https://github.com/insight68/yongxian) - 命令行版本
- [YongXian 主仓库](https://github.com/openclaw/yongxian)
- [社区讨论](https://github.com/insight68/yongxian/discussions)

---

<p align="center">
  Made with ❤️ by <a href="https://github.com/insight68">insight68</a>
</p>
